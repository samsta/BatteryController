...
const can::DataFrame TEST_FRAMES[]{
   // voltages
   can::DataFrame(0x6F2, arr8({0x00, 0x88, 0xF4, 0x25, 0x1D, 0x49, 0x5F, 0xD2}).begin()),
   can::DataFrame(0x6F2, arr8({0x01, 0x8D, 0xB4, 0x21, 0x2D, 0x49, 0x73, 0xD2}).begin()),
   can::DataFrame(0x6F2, arr8({0x02, 0x94, 0xF4, 0x25, 0x5D, 0x49, 0x3F, 0xD2}).begin()),
   can::DataFrame(0x6F2, arr8({0x03, 0x93, 0x74, 0x28, 0x8D, 0x49, 0x73, 0xD2}).begin()),
   can::DataFrame(0x6F2, arr8({0x04, 0x95, 0x34, 0x24, 0xCD, 0x48, 0x5B, 0xD2}).begin()),
   can::DataFrame(0x6F2, arr8({0x05, 0x8E, 0xF4, 0x24, 0xCD, 0x48, 0x0F, 0xD2}).begin()),
   can::DataFrame(0x6F2, arr8({0x06, 0x8C, 0x34, 0x27, 0x3D, 0x49, 0x67, 0xD2}).begin()),
   can::DataFrame(0x6F2, arr8({0x07, 0x93, 0xF4, 0x23, 0xED, 0x47, 0x2B, 0xD2}).begin()),
   can::DataFrame(0x6F2, arr8({0x08, 0x83, 0x34, 0x22, 0xFD, 0x47, 0xF7, 0xD1}).begin()),
   can::DataFrame(0x6F2, arr8({0x09, 0x7F, 0xB4, 0x23, 0x8D, 0x48, 0x27, 0xD2}).begin()),
   can::DataFrame(0x6F2, arr8({0x0A, 0x82, 0xB4, 0x1F, 0x4D, 0x48, 0x53, 0xD2}).begin()),
   can::DataFrame(0x6F2, arr8({0x0B, 0x8A, 0xB4, 0x24, 0xBD, 0x48, 0x17, 0xD2}).begin()),
   can::DataFrame(0x6F2, arr8({0x0C, 0x93, 0xF4, 0x28, 0x1D, 0x4A, 0x8F, 0xD2}).begin()),
   can::DataFrame(0x6F2, arr8({0x0D, 0xA1, 0xF4, 0x25, 0x7D, 0x49, 0x8F, 0xD2}).begin()),
   can::DataFrame(0x6F2, arr8({0x0E, 0x99, 0x34, 0x28, 0x7D, 0x49, 0x5B, 0xD2}).begin()),
   can::DataFrame(0x6F2, arr8({0x0F, 0x7F, 0xF4, 0x23, 0x9D, 0x48, 0x37, 0xD2}).begin()),
   can::DataFrame(0x6F2, arr8({0x10, 0x83, 0x34, 0x20, 0x8D, 0x48, 0x5F, 0xD2}).begin()),
   can::DataFrame(0x6F2, arr8({0x11, 0x92, 0xF4, 0x25, 0x1D, 0x49, 0x1B, 0xD2}).begin()),
   can::DataFrame(0x6F2, arr8({0x12, 0x84, 0xF4, 0x24, 0xED, 0x48, 0x53, 0xD2}).begin()),
   can::DataFrame(0x6F2, arr8({0x13, 0x89, 0xF4, 0x20, 0x8D, 0x48, 0x4F, 0xD2}).begin()),
   can::DataFrame(0x6F2, arr8({0x14, 0x8E, 0xF4, 0x23, 0xDD, 0x48, 0x1B, 0xD2}).begin()),
   can::DataFrame(0x6F2, arr8({0x15, 0x93, 0x74, 0x28, 0x8D, 0x49, 0x83, 0xD2}).begin()),
   can::DataFrame(0x6F2, arr8({0x16, 0x95, 0x34, 0x25, 0x4D, 0x48, 0x53, 0xD2}).begin()),
   can::DataFrame(0x6F2, arr8({0x17, 0x8E, 0xF4, 0x24, 0x9D, 0x48, 0x1F, 0xD2}).begin()),
   // temperatures           
   can::DataFrame(0x6F2, arr8({0x18, 0xC0, 0x87, 0xEF, 0xC1, 0x7B, 0xC0, 0xFC}).begin()),
   can::DataFrame(0x6F2, arr8({0x19, 0xB0, 0x87, 0xEE, 0xA1, 0x7B, 0x70, 0x1E}).begin()),
   can::DataFrame(0x6F2, arr8({0x1A, 0xBA, 0xC7, 0xE7, 0xD1, 0x79, 0x34, 0x1E}).begin()),
   can::DataFrame(0x6F2, arr8({0x1B, 0x9F, 0x87, 0xDF, 0x41, 0x7C, 0xA8, 0x1D}).begin()),
   can::DataFrame(0x6F2, arr8({0x1C, 0xAF, 0x87, 0xDC, 0xE1, 0x79, 0x10, 0x1E}).begin()),
   can::DataFrame(0x6F2, arr8({0x1D, 0xA7, 0xC7, 0xE1, 0x71, 0x7A, 0x50, 0x1E}).begin()),
   can::DataFrame(0x6F2, arr8({0x1E, 0xB1, 0x07, 0xE5, 0xA1, 0x7B, 0xAC, 0x1E}).begin()),
   can::DataFrame(0x6F2, arr8({0x1F, 0xAE, 0x47, 0xED, 0x91, 0x7B, 0xB4, 0x1E}).begin())
};
