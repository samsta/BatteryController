...
public:
   ...
   virtual void close();
   virtual void open();
   virtual bool inverterCommsOk() const;
   virtual void setInverterComms(bool);
...
public:
   ...
   virtual void setInverterComms(bool comms_ok) { m_inverter_comms_ok = comms_ok; }
...
public:
   ...
   TeensyRelay(char *packname, can::FrameSink& sender, uint32_t canid, logging::Logger* log);
   ~TeensyRelay();

   virtual void setState( enum TeensyRelay::State state );
   virtual bool isEnergized() const;
   virtual void close();
   virtual void open();
