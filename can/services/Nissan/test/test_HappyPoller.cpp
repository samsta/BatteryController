...
TEST(HappyPoller, registersAndDeregistersPollingCallbackForillisecondPeriod)
{
   TestSender sender;
...

   {
      ...
      EXPECT_CALL(timer, registerPeriodicCallback(_, HAPPY_POLLER_CALLBACKms,_)).WillOnce(SaveArg<0>(&invokable));
...
public:
   ...
   class ConstructorExpectation
      ...
      public:
         ...
         {
            EXPECT_CALL(timer, registerPeriodicCallback(_, _,_)).WillOnce(SaveArg<0>(invokable));
         }
