...
{
   EXPECT_TRUE(InverterSleepAwakeCmd(can::StandardDataFrame("80008201#5500000000000000")).valid());
}

TEST(TsunInverterSleepAwakeCmd, invalidInfoType)
{
   EXPECT_FALSE(InverterSleepAwakeCmd(can::StandardDataFrame("80008201#0000000000000000")).valid());
}
...
{
   EXPECT_FALSE(InverterSleepAwakeCmd(can::StandardDataFrame("80008201#55000000000000")).valid());
}

TEST(TsunInverterSleepAwakeCmd, commandEnterSleep)
{
   EXPECT_EQ(InverterSleepAwakeCmd::ENTER_SLEEP,InverterSleepAwakeCmd(can::StandardDataFrame("80008201#5500000000000000")).getCommand());
}

TEST(TsunInverterSleepAwakeCmd, commandQuitSleep)
{
   EXPECT_EQ(InverterSleepAwakeCmd::QUIT_SLEEP,InverterSleepAwakeCmd(can::StandardDataFrame("80008201#AA00000000000000")).getCommand());
}
